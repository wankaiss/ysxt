...
function initAutocomplete(){
	...
	$('input[autocomplete]').each(function() {
		...
		$(this).autocomplete({
			...
			//增加动态参数添加，<input type="text" autocomplete="additiveToLinkage.do" forObject="id" getParams="getParams"/>
			//根据属性getParams定义的函数名称来进行增加条件。
				var params = ''; 
				if(me.attr('getParams')){
					eval("params = "+me.attr('getParams')+"()");
					params += "&";
				}
				$.ajax({
					url : url+params+"key="+me.val(), //encodeUrl()
					dataType: "text",
