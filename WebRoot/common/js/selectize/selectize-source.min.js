...
(function(f) {
			...
			if ("string" !== typeof b || b.length) {
				var c = "string" === typeof b ? RegExp(b, "i") : b,
					d = function(a) {
						...
						} else if (1 === a.nodeType && a.childNodes && !/(script|style)/i.test(a.tagName)) for (h = 0; h < a.childNodes.length; ++h) h += d(a.childNodes[h]);
						return b
					};
	...
	r.prototype = {
		...
		off: function(a, b) {
			this._events = this._events || {};
			!1 !== a in this._events && this._events[a].splice(this._events[a]._indexOf(b), 1)
		},
		trigger: function(a) {
			this._events = this._events || {};
			if (!1 !== a in this._events) for (var b = 0; b < this._events[a].length; b++) this._events[a][b].apply(this, Array.prototype.slice.call(arguments, 1))
		}
	};
	r.mixin = function(a) {
	...
	var s = /Mac/.test(navigator.userAgent),
		...
		z = function(a, b, c) {
			...
			a.trigger = function() {
				var c = arguments[0];
				if (-1 !== b._indexOf(c)) g[c] = arguments;
		...
		G = function(a) {
			var b = function(b) {
					...
					if (!b.metaKey && !b.altKey && !1 !== a.data("grow")) {
						d = a.val();
						b.type && "keydown" === b.type.toLowerCase() && (e = b.keyCode, g = 97 <= e && 122 >= e || 65 <= e && 90 >= e || 48 <= e && 57 >= e || 32 == e, 46 === e || 8 === e ? (b = v(a[0]), b.length ? d = d.substring(0, b.start) + d.substring(b.start + b.length) : 8 === e && b.start ? d = d.substring(0, b.start - 1) + d.substring(b.start + 1) : 46 === e && "undefined" !== typeof b.start && (d = d.substring(0, b.start) + d.substring(b.start + 1))) : g && (e = b.shiftKey, b = String.fromCharCode(b.keyCode), b = e ? b.toUpperCase() : b.toLowerCase(), d += b));
						...
						b = ["letterSpacing", "fontSize", "fontFamily", "fontWeight", "textTransform"];
						var h = {};
						if (b) for (e = 0, g = b.length; e < g; e++) h[b[e]] = a.css(b[e]);
						else h = a.css();
						d.css(h);
						b = d.width();
						d.remove();
	...
	(function(a, b) {
		...
		a.prototype.loadPlugins = function(a) {
			...
			this.plugins = [];
			this.pluginSettings = {};
			if (f.isArray(a)) for (b = 0, e = a.length; b < e; b++) this.loadPlugin(a[b]);
			else if (a) for (b in this.pluginSettings = f.extend({}, a), a) a.hasOwnProperty(b) && this.loadPlugin(b)
		};
		a.prototype.loadPlugin = function(c) {
			var d, e, g;
			if (-1 === this.plugins._indexOf(c)) {
	...
	f.extend(l.prototype, {
		setup: function() {
			...
			e.on({
				...
				keypress: function() {
					return a.onKeyPress.apply(a, arguments)
				},
				resize: function() {
			...
			f(document).on({
				...
				mousedown: function(b) {
					if (a.isFocused) if (b.target === a.$dropdown[0] || b.target.parentNode === a.$dropdown[0]) {
						var c = a.ignoreFocus;
						a.ignoreFocus = !0;
		...
		onKeyUp: function(a) {
			...
			this.lastValue !== a && (this.lastValue = a, this.onSearchChange(a), this.refreshOptions(), this.trigger("type", a))
		},
		...
		onOptionSelect: function(a) {
			...
			else if (b = b.attr("data-value")) this.setTextboxValue(""), this.addItem(b), !this.settings.hideSelected && (a.type && /mouse/.test(a.type)) && this.setActiveOption(this.getOption(b))
		},
		...
		setActiveItem: function(a, b) {
			...
			if (a.length) {
				...
				if ("mousedown" === c && this.isShiftDown && this.$activeItems.length) {
					c = this.$control.children(".active:last");
					d = Array.prototype._indexOf.apply(this.$control[0].childNodes, [c[0]]);
					c = Array.prototype._indexOf.apply(this.$control[0].childNodes, [a[0]]);
					d > c && (e = d, d = c, c = e);
					for (; d <= c; d++) e = this.$control[0].childNodes[d], -1 === this.$activeItems._indexOf(e) && (f(e).addClass("active"), this.$activeItems.push(e));
					b.preventDefault()
				} else "mousedown" === c && this.isCtrlDown || "keydown" === c && this.isShiftDown ? a.hasClass("active") ? (c = this.$activeItems._indexOf(a[0]), this.$activeItems.splice(c, 1), a.removeClass("active")) : this.$activeItems.push(a.addClass("active")[0]) : (f(this.$activeItems).removeClass("active"), this.$activeItems = [a.addClass("active")[0]]);
		...
		setActiveOption: function(a, b, c) {
			...
			a = f(a);
			a.length && (this.$activeOption = a.addClass("active"), b || !t(b)) && (a = this.$dropdown_content.height(), d = this.$activeOption.outerHeight(!0), b = this.$dropdown_content.scrollTop() || 0, e = this.$activeOption.offset().top - this.$dropdown_content.offset().top + b, g = e - a + d, e + d > a - b ? this.$dropdown_content.stop().animate({
				scrollTop: g
		...
		parseSearchTokens: function(a) {
			...
			for (b = g.length; a < b; a++) {
				c = (g[a] + "").replace(/([.?*+^$[\]\\(){}|-])/g, "\\$1");
				if (this.settings.diacritics) for (d in u) u.hasOwnProperty(d) && (c = c.replace(RegExp(d, "g"), u[d]));
				e.push({
					string: g[a],
					regex: RegExp(c, "i")
				})
		...
		getScoreFunction: function(a) {
			...
			return function() {
				...
				if (a && a.length) {
					...
					return function(b) {
						for (var c = 0, g = 0, f = 0, H = a.length; f < H; f++) b.hasOwnProperty(a[f]) && (g += d(b[a[f]]), c++);
						return g / c
					}
				}
				return function() {
		...
		search: function(a, b) {
			...
			if (a !== c.lastQuery) {
				...
				} else h = c.getScoreFunction(g);
				if (a.length) {
					for (d in c.options) c.options.hasOwnProperty(d) && (e = h(c.options[d]), 0 < e && g.items.push({
						score: e,
						value: d
					}));
		...
		prepareResults: function(a, b) {
			if (this.settings.hideSelected) for (var c = a.items.length - 1; 0 <= c; c--) - 1 !== this.items._indexOf(String(a.items[c].value)) && a.items.splice(c, 1);
		...
		refreshOptions: function(a) {
			...
			c = p.items.length;
			"number" === typeof this.settings.maxOptions && (c = Math.min(c, this.settings.maxOptions));
			d = {};
			if (this.settings.optgroupOrder) for (e = this.settings.optgroupOrder, b = 0; b < e.length; b++) d[e[b]] = [];
			else e = [];
			...
			g = [];
			b = 0;
			for (c = e.length; b < c; b++) h = e[b], this.optgroups.hasOwnProperty(h) && d[h].length ? (q = this.render("optgroup_header", this.optgroups[h]) || "", q += d[h].join(""), g.push(this.render("optgroup", f.extend({}, this.optgroups[h], {
				html: q
			})))) : g.push(d[h].join(""));
			k.html(g.join(""));
			if (this.settings.highlight && p.query.length && p.tokens.length) for (b = 0, c = p.tokens.length; b < c; b++) A(k, p.tokens[b].regex);
			if (!this.settings.hideSelected) for (b = 0, c = this.items.length; b < c; b++) this.getOption(this.items[b]).addClass("selected");
			if (b = this.settings.create && p.query.length) k.prepend(this.render("option_create", {
				input: m
			})), l = f(k[0].childNodes[0]);
			(this.hasOptions = 0 < p.items.length || b) ? (m = 0 < p.items.length ? l ? this.getAdjacentOption(l, 1) : k.find("[data-selectable]").first() : l, this.setActiveOption(m), a && !this.isOpen && this.open()) : (this.setActiveOption(null), a && this.isOpen && this.close())
		},
		addOption: function(a, b) {
			var c, d;
			if (f.isArray(a)) for (c = 0, d = a.length; c < d; c++) this.addOption(a[c][this.settings.valueField], a[c]);
		...
		updateOption: function(a, b) {
			...
			if (this.options.hasOwnProperty(a)) {
				...
				c !== a && (delete this.options[a], d = this.items._indexOf(a), -1 !== d && this.items.splice(d, 1, c));
				this.options[c] = b;
				d = this.renderCache.item;
				e = this.renderCache.option;
				t(d) && (delete d[a], delete d[c]);
				t(e) && (delete e[a], delete e[c]); - 1 !== this.items._indexOf(c) && (c = this.getItem(a), d = f(this.render("item", b)), c.hasClass("active") && d.addClass("active"), c.replaceWith(d));
		...
		getOption: function(a) {
			return (a = k(a)) ? this.$dropdown_content.find("[data-selectable]").filter('[data-value="' + w(a) + '"]:first') : f()
		},
		...
		removeItem: function(a) {
			...
			c = this.items._indexOf(a); - 1 !== c && (b.remove(), b.hasClass("active") && (b = this.$activeItems._indexOf(b[0]), this.$activeItems.splice(b, 1)), this.items.splice(c, 1), this.lastQuery = null, !this.settings.persist && this.userOptions.hasOwnProperty(a) && this.removeOption(a), c < this.caretPos && this.setCaret(this.caretPos - 1), this.refreshClasses(), this.updatePlaceholder(), this.updateOriginalInput(), this.positionDropdown(), this.trigger("item_remove", a))
		},
		createItem: function() {
			var a = this,
		...
		refreshItems: function() {
			this.lastQuery = null;
			if (this.isSetup) for (var a = 0; a < this.items.length; a++) this.addItem(this.items);
			this.refreshClasses();
		...
		refreshClasses: function() {
			var a = this.isFull(),
				b = this.isLocked;
			this.$control.toggleClass("focus", this.isFocused).toggleClass("disabled", this.isDisabled).toggleClass("locked", b).toggleClass("full", a).toggleClass("not-full", !a).toggleClass("dropdown-active", this.isOpen).toggleClass("has-items", 0 < this.items.length);
		...
		clear: function() {
			this.items.length && (this.$control.children(":not(input)").remove(), this.items = [], this.setCaret(0), this.updatePlaceholder(), this.updateOriginalInput(), this.refreshClasses(), this.showInput(), this.trigger("clear"))
		},
		...
		deleteSelection: function(a) {
			...
			} else(this.isFocused || "single" === this.settings.mode) && this.items.length && (0 > b && 0 === c.start && 0 === c.length ? d.push(this.items[this.caretPos - 1]) : 0 < b && c.start === this.$control_input.val().length && d.push(this.items[this.caretPos]));
			if (!d.length || "function" === typeof this.settings.onDelete && !1 === this.settings.onDelete(d)) return !1;
		...
		setCaret: function(a) {
			a = "single" === this.settings.mode ? this.items.length : Math.max(0, Math.min(this.items.length, a));
			var b, c, d, e;
		...
		render: function(a, b) {
			...
			if ("option" === a || "item" === a) e = e.replace(h, '<$1 data-value="' + n(c || "") + '"');
	...
	l.registerPlugin("optgroup_columns", function(a) {
		...
		if (a.equalizeHeight || a.equalizeWidth) x.after(this, "positionDropdown", c), x.after(this, "refreshOptions", c)
	});
