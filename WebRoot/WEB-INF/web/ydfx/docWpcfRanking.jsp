...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>医生开外配处方次数、费用排名（总的排名）</title>
...
<body>
	...
	<form action="hztj_docWpcfRanking.action" method="post" name="form" id="chooseForm" target="_self">
						...
						<s:if test="null != pageSet.data && pageSet.data.size()>0">
							<s:iterator value="pageSet.data" var="cons" status="index">
								<tr>
									...
									<td id="ckmx">
										<a href="javascript:void;" onclick="viewItem();">查看明细</a>
									</td>
								</tr>
							</s:iterator>
						</s:if>
	...
	$(document).ready(function(){
		...
		function query(){
			...
			checkDate();
			$("#start").val(0);
			form.action="hztj_docWpcfRanking.action";
			form.target='_self';
			form.submit();
		}
		...
		function exportExcel(){
			alert("正在导出中......请稍后!");
			form.action = "ydfx_exportExcelDocWpcfRanking.action";
			form.target='_self';
			form.submit();
		}
		...
		function viewItem(){
			...
			$("#trId tr").click(function(){
			    var tr = $(this);
			    var ydName = tr.context.cells[1].innerText;
			    var docId = tr.context.cells[2].innerText;
			    var docName = tr.context.cells[3].innerText;
			    var wndFeatures = "width=850px;height=450px;scrollbars=yes;resizable=no;status=no;toolbar=no;";
				window.open(encodeURI(encodeURI('ydfx_docWpcfDetail.action?condition.docId='+docId+"&condition.startDate="+startDate+"&condition.endDate="+endDate+"&condition.akb021="+docName+"&condition.ydNames="+ydName)),"医生明细",wndFeatures);
			});
		}
	</script>
</body>
