...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		<title>单个医生的中药饮片费用及贴均价格查询</title>
		...
		<!-- overflow-x: hidden;
		 	overflow-y:scroll;
		 	style="overflow-x:srolll; overflow-y:hidden; height:480px"-->
...
<body>
	...
	<form action="zyyp_singleDoctor.action" method="post" name="form" id="chooseForm" target="_self">
		<div class="dateBox winscroll"  style="clear: both;">
			<div class="tab_nr">
				<div class="wd-content-box">
					<table class="date_tab" width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							...
							<%--<td>排名数量：</td>
							<td>
								<input type="text" name="condition.pmsl" id="" vlaue=""/>
							</td>
						--%>
						</tr>
						<tr>
							...
							<td style="text-align: left;">
								<select name="condition.akb022" id="akb022" style="width:100px;">
							...
							<td style="text-align: left;">	
								...
								<input name="condition.hosNames" id="hosNames" value="${condition.hosNames}" onclick="openSelectWindow();" readonly="readonly" style="width:100px;"/>
							</td>
							<td>医生名称：</td>
							<td style="text-align: left;">
				...
				<div class="wd-content-box" >
					<table class="date_tab" style="width : 3150px; " border="0" cellspacing="0" cellpadding="0" id="trId" >
						<thead>
							<tr>
						...
						<s:if test="null != pageSet.data && pageSet.data.size()>0">
							<s:iterator value="pageSet.data" var="cons" status="index">
								<tr>
									...
									<td>
										<c:if test="${empty cons.XZLB_1 }">
											${akc021Map[cons.XZLB_2] }
										</c:if>
										<c:if test="${empty cons.XZLB_2 }">
											${akc021Map[cons.XZLB_1] }
										</c:if>
									...
									<td>
										<fmt:formatNumber value="${cons.TBFWPTS }" pattern="#,###" minFractionDigits="0"/>
									</td>
									<td>
									...
									<td>
										<fmt:formatNumber value="${cons.TBWPTS }" pattern="#,###" minFractionDigits="0"/>
									</td>
									<td>
									...
									<td>
										<fmt:formatNumber value="${cons.FTBFWPTS }" pattern="#,###" minFractionDigits="0"/>
									</td>
									<td>
									...
									<td>
										<fmt:formatNumber value="${cons.FTBWPTS }" pattern="#,###" minFractionDigits="0"/>
									</td>
									<td>
	...
	<script type="text/javascript">
		...
		function query(){
			...
			checkDate();
			$("#start").val(0);
			form.action = "zyyp_singleDoctor.action";
			form.target = '_self';
			form.submit();
		}
		//日期校验
		...
		function docSelect(){
			...
			var aaa027 = $("#aaa027").val();
			var wndFeatures = "width=850px;height=450px;scrollbars=yes;resizable=no;status=no;toolbar=no;";
			window.open('zyyp_docSelect.action?condition.hosIds='+hosIds+"&condition.aaa027="+aaa027,"医师查询",wndFeatures);
		}
		
		//区划重新选择时清空医院和医师已选的信息
		function setEmptyValue(e){
			//如果选中请选择则不变否则清空医院和医师已选的信息
			if(e!=""){
				$("#hosIds").val("");
				$("#hosNames").val("");
				$("#ydIds").val("");
		...
		function exportExcel(){
			form.action = "zyyp_exportExcelDoctor.action";
			form.target = '_self';
			form.submit();
		}
	</script>
