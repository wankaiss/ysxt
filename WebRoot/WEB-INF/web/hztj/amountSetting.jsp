...
<body>
	<form action="hztj_amountSetting.action" name="form" method="post" id="chooseForm" target="_self">
			...
			<div class="tab_nr">
				...
				<div class="wd-content-box">
					<table class="date_tab" id="trId" width="95%" border="0" cellspacing="0" cellpadding="0">
						...
						<s:if test="null != pageSet.data && pageSet.data.size()>0">
							<s:iterator value="pageSet.data" var="cons" status="index">
								<tr>
									...
									<%--<td style="text-align: center;">
										<a href="#"  onclick="weigthUpd('je','<s:property value="#cons.QZZ"/>','<s:property value="#cons.ID"/>');">费用额度修改</a>
									</td>--%>
								</tr>
							</s:iterator>
						</s:if>
						<s:else>
							<tr>
								<td style="text-align: center;">单次非处方购药当年账户支付费用额度</td>
								<td>
									<input type="text" id="cf" name="condition.qzz" value=""/>元
								</td>
							</tr>
							<tr>
								<td style="text-align: center;">单次非处方购药历年账户支付费用额度</td>
								<td>
									<input type="text" id="fcf" name="condition.qzz" value=""/>元
								</td>
							</tr>
						</s:else>
					</table>
					<div style="text-align: center;">
						<s:if test="null != pageSet.data && pageSet.data.size()>0">
							<input type="button" value="费用额度修改" onclick="changeWeigth();"/>
						</s:if>
						<s:else>
	...
	<script type="text/javascript">
		...
		function saveData(){
			...
			$.ajax({
				url:"hztj_saveParameterAmount.action",
				cache:false,
				type:"post",
				data:{
					tempqxm:tempqxm,
		...
		function query(){
			form.action = "hztj_amountSetting.action";
			form.target = "_self";
			form.submit();
		}
		...
		function changeWeigth(){
			...
			var k = window.showModalDialog('hztj_changeWeigth.action?condition.gzbz='+gzbz+"&condition.qzjb=6"+"&condition.tcdm="+temptcdm+"&condition.qzdm="+qzdm,null,wndFeatures);
			if(k){
				//修改成功重新加载页面
				query();
			}
