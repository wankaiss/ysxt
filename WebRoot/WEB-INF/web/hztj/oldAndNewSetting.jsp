...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		<title>当年账户历年账户权重设置</title>
		<link href="${path}css/public.css" rel="stylesheet" type="text/css" />
		<link href="${path}css/style.css" rel="stylesheet" type="text/css" />
		<script type="text/javascript" src="${path}js/jquery-1.11.0.min.js"></script>
	</head>
<body>
	<form action="hztj_oldAndNewSetting.action" name="form" method="post" id="chooseForm" target="_self">
	...
	<script type="text/javascript">
		...
		function saveData(){
			...
			$.ajax({
				url:"hztj_saveParameterOldAndNew.action",
				cache:false,
				type:"post",
				data:{
					tempqxm:tempqxm,
		...
		function query(){
			form.action = "hztj_oldAndNewSetting.action";
			form.target = "_self";
			form.submit();
		}
		...
		function changeWeigth(){
			...
			var k = window.showModalDialog('hztj_changeWeigth.action?condition.gzbz='+gzbz+"&condition.qzjb=5"+"&condition.tcdm="+temptcdm+"&condition.qzdm="+qzdm,null,wndFeatures);
			if(k){
				//修改成功重新加载页面
				query();
			}
